	...
	SubShader
	{
		...
		Pass
		{
			...
			PixelOut p(VertexOut IN)
			{
				...
				float4 projColor = tex2D(_ShadowMap, uv);

				float4 realColor = tex2D(_MainTex, IN.uv);

				float4 c = lerp(realColor, projColor, all(float2(step(0.0, min(uv.x, uv.y)), step(max(uv.x, uv.y), 1.0))));
				/*float4 c = projColor;
				if( uv.x < 0 || uv.y < 0 ||
